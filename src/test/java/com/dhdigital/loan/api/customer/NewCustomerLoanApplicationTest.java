...
//	@EndpointInject("mock://http:localhost:8080/api/loan/v1/NewLoanApplication")
//	private MockEndpoint cdmockEndpoint;
	...
	public void newCustomerLoanApplication_Success() throws Exception{
		...
		String newCustomerLoanApplicationRequest = Resources.toString(
                Resources.getResource("mock/frontend/NewCustomerLoanApplication/request/NewCustomerLoanApplicationRequest.json"), Charsets.UTF_8);

        String newCustomerLoanApplicationResponseBknd = Resources.toString(
                Resources.getResource("mock/backend/NewCustomerLoanApplication/response/NewCustomerLoanApplicationResponseBknd.json"), Charsets.UTF_8);
		...
		NewLoanApplicationRequestType oNewLoanApplicationRequestType = objectMapper.readValue(newCustomerLoanApplicationRequest, NewLoanApplicationRequestType.class);
		
		NewLoanApplicationResponseType oNewLoanApplicationResponseType = producerTemplate.requestBody("direct:newCustomerLoanApplication", oNewLoanApplicationRequestType, NewLoanApplicationResponseType.class);				
		
		System.out.println("Response: "+ oNewLoanApplicationResponseType.getNewLoanApplicationResponse().getLoanStatus().toString());
		
		Assertions.assertTrue(null != oNewLoanApplicationResponseType.getNewLoanApplicationResponse().getLoanStatus().toString());
	}
